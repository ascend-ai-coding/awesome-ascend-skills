...
def analyze_slow_rank_results(db_path):
    """分析快慢卡检测结果"""
    print(f"\n📊 快慢卡检测结果分析")
    print("-" * 80)
    ...
    try:
        ...
        # 查询所有rank的快慢卡影响次数
        cursor.execute("SELECT rankId, slowAffectCount FROM slow_rank_analysis")
        ...
        for rank_id, count in results:
            ...
            if count == 0:
                severity = "正常"
            elif (count >= 1 and count <= 5) or (z_score > 1.5 and z_score <= 2):
                severity = "轻微慢卡"
            elif (count >= 6 and count <= 20) or (z_score > 2 and z_score <= 3):
                severity = "中度慢卡"
        ...
        if slow_ranks:
            ...
            if max_affected_rank[1] > avg_count and avg_count > 0:
                print(f"超过平均值: {(max_affected_rank[1] - avg_count):.2f} ({((max_affected_rank[1] - avg_count)/avg_count*100):.1f}%)")
        else:
            print("\n✅ 未检测到明显的快慢卡问题")
        ...
        print("-" * 80)
        if not slow_ranks:
            print("1. 保持当前配置，定期监控性能")
            print("2. 考虑进一步优化通信与计算的重叠")
            print("3. 检查内存访问模式，优化数据传输")
        else:
            # 检查是否有严重慢卡
...
def main():
    ...
    analyze_slow_rank_results(db_path)
