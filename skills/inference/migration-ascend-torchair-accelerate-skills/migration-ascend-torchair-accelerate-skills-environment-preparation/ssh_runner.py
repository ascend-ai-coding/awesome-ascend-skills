...
用法:
    ...
    run_remote("docker exec xxx python3 test.py", host="175.100.2.6", 
               user="root", password="Huawei@123", deadline=600)
"""
...
def get_runner(host="175.100.2.6", user="root", password="Huawei@123"):
...
def run_remote(cmd, deadline=600, host="175.100.2.6", user="root", password="Huawei@123",
...
if __name__ == "__main__":
    ...
    host = sys.argv[2] if len(sys.argv) > 2 else "175.100.2.6"
    user = sys.argv[3] if len(sys.argv) > 3 else "root"
    pwd = sys.argv[4] if len(sys.argv) > 4 else "Huawei@123"
