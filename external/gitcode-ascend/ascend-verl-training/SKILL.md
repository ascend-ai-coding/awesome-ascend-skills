---
name: external-gitcode-ascend-ascend-verl-training
...
original-name: ascend-verl-training
...
```
ascend-verl-training (主skill)
├── ascend-verl-env-preparation: 环境准备 (拉取镜像、创建启动脚本、启动容器)
├── ascend-verl-image-list: 询 quay.io 上可用的 VERL 镜像
├── ascend-verl-env-check: 环境检查 (容器状态、依赖工具)
├── ascend-verl-prepare-data: 数据集处理
├── ascend-verl-model-download: 模型下载
└── train-runner: 执行训练
```
...
ENFORCE_CHECK:
  ...
  - USE_SKILL: 必须使用 "skill ascend-verl-env-preparation(...)" 调用，禁止直接执行bash

在收集用户配置时，检测用户是否提到"复用"关键词（如"复用"、"使用之前的容器"等），设置 `REUSE_CONTAINER=true`，否则为 `false`。

调用 `ascend-verl-env-preparation` skill，传递参数：
```bash
skill ascend-verl-env-preparation(VERL_IMAGE, HTTP_PROXY, HTTPS_PROXY, VERL_WORK_DIR, VERL_DATA_DIR, VERL_MODEL_DIR, VERL_CONTAINER, ROLLOUT_NAME, REUSE_CONTAINER)
```
...
ENFORCE_CHECK:
  ...
  - USE_SKILL: 必须使用 "skill ascend-verl-env-check(...)" 调用，禁止直接执行bash

调用 `ascend-verl-env-check` skill，传递参数：
```bash
skill ascend-verl-env-check(VERL_CONTAINER, HTTP_PROXY, HTTPS_PROXY)
```
...
ENFORCE_CHECK:
  ...
  - USE_SKILL: 必须使用 "skill ascend-verl-prepare-data(...)" 调用，禁止直接执行bash

调用 `ascend-verl-prepare-data` skill，传递参数：
```bash
skill ascend-verl-prepare-data(DATASET_NAME, VERL_DATA_DIR, HTTP_PROXY, HTTPS_PROXY, VERL_CONTAINER)
```
...
ENFORCE_CHECK:
  ...
  - USE_SKILL: 必须使用 "skill ascend-verl-model-download(...)" 调用，禁止直接执行bash

调用 `ascend-verl-model-download` skill，传递参数：
```bash
skill ascend-verl-model-download(MODEL_NAME, VERL_MODEL_DIR, HTTP_PROXY, HTTPS_PROXY, VERL_CONTAINER)
```
...
| Qwen2.5-0.5B-Instruct GRPO训练脚本 | `/workspace/.opencode/skills/ascend-verl-training/scripts/run_*.sh` |
...
| 镜像拉取失败 | 检查网络/代理设置，重试 |
| 容器启动失败 | 检查Docker状态 |
| 容器不存在 | 调用 ascend-verl-env-preparation |
| 依赖缺失 | 调用 ascend-verl-env-check 查看详情 |
