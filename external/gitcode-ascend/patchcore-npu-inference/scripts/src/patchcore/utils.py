...
def set_torch_device(gpu_ids, use_npu=False, npu_device_id=0):
    """Returns correct torch.device.

    Args:
        gpu_ids: [list] list of gpu ids. If empty, cpu is used.
        use_npu: [bool] If true, an NPU device is returned.
        npu_device_id: [int] NPU device id.
    """
    if use_npu:
        return torch.device("npu:{}".format(npu_device_id))
    if len(gpu_ids):
        return torch.device("cuda:{}".format(gpu_ids[0]))
    return torch.device("cpu")


def fix_seeds(seed, with_torch=True, with_npu=False):
    """Fixed available seeds for reproducibility.

    Args:
        seed: [int] Seed value.
        with_torch: Flag. If true, torch-related seeds are fixed.
        with_npu: Flag. If true, torch+npu-related seeds are fixed
    """
    random.seed(seed)
    np.random.seed(seed)
    if with_torch:
