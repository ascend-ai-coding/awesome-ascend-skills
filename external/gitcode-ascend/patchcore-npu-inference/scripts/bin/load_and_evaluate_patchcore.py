...
@click.option("--use_npu", is_flag=True, default=False)
@click.option("--npu_device_id", type=int, default=0)
@click.option("--use_fp16", is_flag=True, default=False)
def main(**kwargs):
    pass


@main.result_callback()
def run(methods, results_path, gpu, seed, save_segmentation_images, use_npu, npu_device_id, use_fp16):
    methods = {key: item for (key, item) in methods}

    os.makedirs(results_path, exist_ok=True)

    device = patchcore.utils.set_torch_device(gpu, use_npu, npu_device_id)
    # Device context here is specifically set and used later
    # because there was GPU memory-bleeding which I could only fix with
    # context managers.
    device_context = (
        torch.npu.device("npu:{}".format(device.index))
        if "npu" in device.type.lower()
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        _is_npu = "npu" in device.type.lower()
        patchcore.utils.fix_seeds(seed, with_torch=True, with_npu=_is_npu)
...
@click.option("--npu_device_id", type=int, default=0)
@click.option("--use_fp16", is_flag=True, default=False)
def patch_core_loader(patch_core_paths, faiss_on_gpu, faiss_num_workers, use_npu, npu_device_id, use_fp16):
    def get_patchcore_iter(device):
        for patch_core_path in patch_core_paths:
            ...
            if use_npu:
                ...
                nn_method = BaihuNN(on_npu=True, device_id=npu_device_id, use_fp16=use_fp16)
            else:
