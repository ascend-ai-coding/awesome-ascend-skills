---
name: external-gitcode-ascend-ascend-verl-model-download
...
传递参数时不交互：
```bash
skill ascend-verl-model-download(
...
```
[主skill调用ascend-verl-model-download]
参数: MODEL_NAME=Qwen2.5-0.5B-Instruct, VERL_MODEL_DIR=/home/model_weights
执行: skill ascend-verl-model-download(MODEL_NAME, VERL_MODEL_DIR, HTTP_PROXY, HTTPS_PROXY, VERL_CONTAINER)
结果: ready ✓
```
