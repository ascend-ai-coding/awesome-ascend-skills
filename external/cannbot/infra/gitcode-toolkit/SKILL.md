...
description: GitCode 协作通用基础参考（内部参考，不直接触发）。提供 GitCode API、Token 配置、URL 解析、日志规范、变更展示，Git
  克隆/分支/diff/log/remote 通用操作，以及 PR 创建、Issue 创建和 Issue 评论工作流等共享文档与确定性脚本。供 gitcode-pr-handler、gitcode-issue-gen、gitcode-issue-handler
  等 GitCode 协作类 skill 引用使用，本 skill 自身不响应用户触发。
