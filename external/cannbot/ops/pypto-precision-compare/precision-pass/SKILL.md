---
name: external-cannbot-ops-pypto-precision-compare-precision-pass
description: Pass精度校验子技能。开启PreCheck/PostCheck进行全链路Pass校验，通过pass校验定位报错Pass，使用pass_compare逐Op对比定位具体问题Op，支持动态shape上板数据打印验证。适用于
  Pass 精度校验并定位问题 Op 时。
original-name: precision-pass
